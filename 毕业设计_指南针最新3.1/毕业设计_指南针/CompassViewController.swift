...
//
        ...
        angelLabel.frame = CGRect(x: deviceWidth * 0.1, y: zhiNanZhenImageWH + zhiNanZhenImagePointY , width: deviceWidth * 0.4, height: deviceHeight * 0.2)
...
//            self.yawLabel.text = String(format: "%.2f", yaw!)
            UIView.animateWithDuration(0.1, delay: 0, options: UIViewAnimationOptions.AllowUserInteraction, animations: { () -> Void in
                ...
                self.levelImageView.center.x = centerX + CGFloat(-roll! * 60)
                self.levelImageView.center.y = centerY + CGFloat(-pitch! * 60)
